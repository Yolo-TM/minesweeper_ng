...
const DEFAULT_PROGRESS_TEMPLATE: &str =
    "Overall: [{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} ({percent}%) | {wide_msg}";
const WORKER_PROGRESS_TEMPLATE: &str =
    "Worker {}: {spinner:.green} [{bar:40.cyan/blue}] {pos}/{len} ({percent}%)";
...
pub fn generate_fields(field_data: CommandResult) {
    ...
    println!(
        "Using {} worker threads with {} fields per worker_count\n",
    ...
    overall_progress.set_style(
        ProgressStyle::with_template(DEFAULT_PROGRESS_TEMPLATE)
            .unwrap()
            .progress_chars(PROGRESS_CHARS),
    );
    overall_progress.set_message("Starting generation...");

    start_workers(worker_count, workload, &field_data, tx, &multi_progress);
...
fn worker(
    ...
    progress_bar.set_style(
        ProgressStyle::with_template(
            &WORKER_PROGRESS_TEMPLATE.replace("{}", &worker_id.to_string()),
        )
        .unwrap()
        .progress_chars(PROGRESS_CHARS),
    );
    for id in ids {
        if field_data.no_guess {
            ...
            field.to_file(&filename).unwrap_or_else(|err| {
                multi_progress
                    .println(format!("Error writing to file {}: {}", filename, err))
                    .unwrap();
            });
        } else {
