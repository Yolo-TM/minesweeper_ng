...
Dirty copy paste currently, needs cleanup

Also this sometimes hits a mine, there is still an edge case not covered
...
pub fn solve(solver: &Solver) -> (Vec<(u32, u32)>, Vec<(u32, u32)>) {
    ...
    for (x, y) in solver.sorted_fields() {
        ...
        let count = solver.get_reduced_count(x, y) as usize;
        let surrounding_hidden_fields = solver.get_surrounding_unrevealed(x, y);
    ...
    for i in 0..boxes.len() {
        ...
        for j in 0..boxes.len() {
            ...
            if shared.len() > 0
                && shared.len() >= other.len()
                && !(box_.covers_same_fields(other_box) && box_.has_same_range(other_box))
            {
        ...
        for j in 0..boxes.len() {
            ...
            if possible_fields
                .iter()
                .any(|b| b.get_owner() == boxes[j].get_owner())
            {
                continue;
            }
            ...
            if possible_fields
                .iter()
                .any(|b| b.covers_same_fields(&boxes[j]) && b.has_same_range(&boxes[j]))
            {
                continue;
            }
        ...
        if !possible_fields.is_empty() {
            ...
            if possible_fields
                .iter()
                .all(|field| field.get_mines() == range)
            {
                allow_insert = false;
            }
    ...
    for (original, adjacent) in possible_cases {
        let mut field_tuples: Vec<(std::ops::RangeInclusive<usize>, Vec<(u32, u32)>)> =
            vec![(original.get_mines(), original.get_fields().clone())];
        let mut safe = vec![];
        let mut mines = vec![];
        ...
        if safe.is_empty() && mines.is_empty() {
            ...
            deep_search(solver, &all_boxes, &mut safe, &mut mines);
        }
...
fn recursive_search(
    ...
    let box_ = boxes[current_box_index].clone();
    {
        ...
        for i in 0..field_tuples.len() {
            ...
            let mut new_shared_range = second_range.start().saturating_sub(this_only.len())
                ..=*second_range.end().min(&shared.len());
            let mut new_other_range = range.start().saturating_sub(*new_shared_range.end())
                ..=range.end().saturating_sub(*new_shared_range.start());

            if (new_shared_range.end() > new_other_range.end())
                || (shared.len() == 1 && new_shared_range.start() != new_shared_range.end())
                || (other_only.len() == 1
                    && *new_other_range.start() == 0
                    && *new_other_range.end() == 1)
            {
                ...
                // stop
                continue;
            } else if other_only.len() == 1
                && new_other_range.start() != new_other_range.end()
                && *new_other_range.start() != 0
            {
                ...
                if did_redistribute_mines(excess_mines, &mut new_shared_range, field_tuples, i) {
            ...
            new_tuples.push((new_other_range, other_only.clone()));

            new_tuples.push((new_shared_range, shared.clone()));
        }
...
fn deep_search(
    ...
    let valid_permutations = generate_all_permutations(solver, &mut all_fields, &boxes);
...
fn generate_all_permutations(
    ...
    let mut field_vec: Vec<((u32, u32), bool)> =
        all_fields.iter().map(|(&k, &v)| (k, v == 1)).collect();
    let mut permutations = 0;

    recursively_generate_permutations(
        solver,
        all_fields,
        &mut field_vec,
        0,
...
fn recursively_generate_permutations(
    ...
    if index == field_vec.len() {
        check_permutation(field_map, permutations, field_vec, boxes);
        return;
    }
    ...
    recursively_generate_permutations(solver, field_map, field_vec, index + 1, boxes, permutations);
...
fn check_permutation(
    ...
    let lookup_map: HashMap<(u32, u32), bool> = field_vec
        .iter()
...
fn did_redistribute_mines(
    ...
    if remaining_excess > 0 {
        distribute_excess_to_other_tuples(&mut remaining_excess, field_tuples, current_tuple_index);
    }
